...
top-level ``from agents.models import Agent`` cross-app import.
...
- is_agent_offline: status helper (replaces Agent.Status.OFFLINE ref in accounts)
"""
...
def create_agent_token(name: str, permissions: list[str]) -> tuple[Agent, str]:
...
def revoke_agent_token(pk: int) -> Agent | None:
...
def get_agent_for_device_check(device_id: int) -> Agent | None:
    ...
    """
    try:
        return Agent.objects.get(pk=device_id)
    except Agent.DoesNotExist:
        return None


def is_agent_offline(agent: Agent) -> bool:
    """Return True if the agent's status is OFFLINE.

    Helper for callers (e.g. accounts.views.GameAccountViewSet.test_login)
    that need to check offline status without importing ``Agent.Status``.
    """
    return agent.status == Agent.Status.OFFLINE
