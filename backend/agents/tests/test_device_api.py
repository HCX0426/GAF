...
from agents.factories import AgentFactory, DeviceFactory, DeviceGroupFactory
from agents.models import Device, DeviceGroup
...
class TestDeviceListFilter(TestCase):
    ...
    def setUp(self):
        """初始化测试数据：操作员用户、多个不同类型的设备、API 客户端"""
        self.operator = OperatorUserFactory()
...
class TestDeviceUpdateDelete(TestCase):
    ...
    def test_device_detail_with_agent_info(self):
        """设备详情包含 agent_info"""
        agent = AgentFactory(agent_id='detail-agent', hostname='detail-host')
...
class TestDeviceScan(TestCase):
    ...
    def setUp(self):
        ...
        self.agent = AgentFactory(agent_id='scan-agent', hostname='scan-host')
        self.client = APIClient()
...
class TestDeviceRegister(TestCase):
    ...
    def test_test_screenshot(self):
        """测试截图 → 设备存在时应返回正确结构（截图模块可能不可用）"""
        agent = AgentFactory(agent_id='test-screenshot-agent', hostname='test-host')
    ...
    def test_test_screenshot_with_method_param(
        ...
        agent = AgentFactory(agent_id='test-screenshot-method-agent', hostname='test-host')
        device = DeviceFactory(
            name='截图方式测试设备',
            device_type='emulator',
            status='online',
