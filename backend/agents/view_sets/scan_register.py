...
from agents.services import _invalidate_available_methods_cache
from agents.view_sets.crud import DeviceViewSet
...
class DeviceScanView(APIView):
    ...
    def _select_agent(self, agent_id=None):
        ...
        return Agent.objects.filter(
            Q(status=Agent.Status.ONLINE) | Q(status=Agent.Status.IDLE),
        ).first()
...
class DeviceRegisterView(APIView):
    ...
    def post(self, request: Request):
        ...
        try:
            local_agent = Agent.objects.filter(is_local=True).first()
            if local_agent:
