"""WebSocket payload compressor: MessagePack + zlib for large frames.

✅ Status (spec-42, TD-287): helper mirrors
``backend/protocol/message_compressor.py`` so both sides of the WebSocket
share the same wire format. The agent uses it after the Hello/Hello.ack
negotiation completes; legacy servers that don't participate in the
negotiation fall back to JSON text_data.
...
--------------------
1. Serialize: try ``msgpack`` (binary, ~30% smaller than JSON for nested
   dicts), fall back to ``json.dumps`` when msgpack is not installed.
2. Compress: when serialized bytes exceed ``compress_threshold`` (default
   1024 bytes), apply ``zlib.compress``. Smaller frames are sent raw to
   avoid per-frame zlib overhead.
...
::
    ...
    wire_bytes = compressor.compress({"type": "task.result", "payload": {...}})
    payload = compressor.decompress(wire_bytes)  # → original dict

Drift mitigation
----------------
Backend and agent must agree on the wire format. If you change a constant
or a helper signature here, mirror the change in
``backend/protocol/message_compressor.py`` and update the unit tests on
both sides. The agent-side ``test_message_compressor.py`` and backend-side
``test_message_compressor.py`` intentionally mirror each other so drift
fails one or both test suites.
"""
...
def build_hello_frame(
    ...
    Returns:
        ...
        serializing it (typically via ``json.dumps``) and sending as
        ``text_data`` on the WebSocket.
...
def parse_hello_capabilities(frame: dict[str, Any]) -> tuple[list[str], int]:
    ...
    Args:
        frame: Parsed frame dict (e.g. from ``json.loads``). Must have
            ``type == "hello"``.
