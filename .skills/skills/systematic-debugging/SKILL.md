...
> 本 skill 是 superpowers 通用方法论, 不绑定任何项目。
> 在 GAF 项目中使用时, 遵循以下边界 (TD-029 闭环):

- **入口约束**: 不直接调用, 由 `gaf-orchestrator` 决策树在 task_type=bug_fix 时加载
- **职责边界**: 本 skill 主导"修复前根因调查"; 修复完成 commit 后的"反思与教训收集"由 `gaf-reflect-and-evolve` (GAF 项目特定) 主导
- **时间轴**: bug 发生 → 本 skill (根因+修复) → commit → gaf-reflect-and-evolve (反思+教训)
- **GAF 项目详细集成规范**: 见 `../gaf-reflect-and-evolve/SKILL.md §6.2`
