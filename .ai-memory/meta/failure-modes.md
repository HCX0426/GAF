...
| N217 | 登录"记住我"失效 (2026-08-29 用户反馈"不是可以记住账号吗，为啥每次都要登录") — 跨功能迭代基础流程回归盲区 + antd Form.Item 间接子元素坑 | 跨功能迭代时把用户高频基础流程(登录/注册/鉴权恢复)纳入每轮回归清单; antd Form.Item 直接子元素必须是受控组件(含 name + valuePropName 的 Checkbox/Input), 否则 value/onChange 不注入该组件, UI 显示与表单值脱节; 详见 lesson N217 | `lessons/workflow_2026-08-29_n217-login-remember-me-regression.md` | 1 | 2026-08-29 |
