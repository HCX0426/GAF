...
| `services/health.py` | spec 2026-08-29 service health probes: 4 app-level checks (redis PING / backend /api/v2/system/healthz/ / agent DB heartbeat / frontend HTTP), writes `debug/health-status.json` (+ process/error scan `scan_log_errors`); consumed by gaf_daemon watchdog + monitors/status + monitors/services. | `python scripts/services/health.py --check --write` | import by `gaf_daemon.py` (`_run_health_checks`) |
