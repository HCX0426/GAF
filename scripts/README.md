...
| `gaf_daemon.py` | TD-352 service daemon: process manager + watchdog auto-restart (start/stop/restart/status/monitor), replaces gaf_services.ps1 orchestration. PID: `debug/gaf_daemon.pid`. | `python scripts/gaf_daemon.py start` / `status` / `stop` | Manual + `gaf_services.ps1` integration |
| `services/health.py` | spec 2026-08-29 service health probes: 4 app-level checks (redis PING / backend /api/v2/system/healthz/ / agent DB heartbeat / frontend HTTP), writes `debug/health-status.json`; consumed by gaf_daemon watchdog + monitors/status. | `python scripts/services/health.py --check --write` | import by `gaf_daemon.py` (`_run_health_checks`) |
