...
| P4 | 验收：accounts 全量测试 + 账户 e2e + 全量回归 + game_name 引用残留清零 | — | ✅ 已验收（accounts/gamestate/tasks 336 passed @ f4fbbea；后端 game_name 残留仅剩 GameProfile 字段 / game_name_display / 收敛迁移，无 GameStateRule·GameVersionCheck·MarketplaceItem 字符串字段）|
...
| P7 | **扩展**: MarketplaceItem.game_name → game_profile FK + 前端去硬编码游戏选项 | ~150 行 + 前端 | ✅ 后端 b67b34c；前端 f4fbbea（Marketplace.tsx 改从 fetchGameProfiles 拉取选项 + publish 发 game_profile id）|
| P8 | 验收(扩展)：gamestate/tasks 全量测试 + game_name 引用清零 + 归档 | — | ✅ 已验收并归档（336 passed；归档至 docs/specs/archived/2026-08/）|
