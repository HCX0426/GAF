...
| 项 | 现状 | 目标 |
|----|------|------|
| `ChainManager`(`worker/src/engine/chain_manager.py:20`) | `StateMachine` 封装，名不副实 | `StateMachineEngine`(+`state_machine_engine.py`) |
...
- `worker/src/engine/chain_manager.py`→`state_machine_engine.py`；class `ChainManager`→`StateMachineEngine`（含内部引用）。
...
- `conda run -n gaf python -m pytest worker/tests backend/scheduler -q` 通过。
