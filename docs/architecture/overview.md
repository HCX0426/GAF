...
> **架构图**: [system-overview.svg](./system-overview.svg) — 全栈架构一图总览（前端 → 后端 → Agent → 设备 + AI 治理层）
> **状态**: 核心架构重构已完成 (Phase 1-3 ✅) + 设备自动关联已实施 (Phase R16 ✅ + Phase R18 ✅) + 跨平台层落地 (P-028 ✅) + AI Agent 集成 (LangGraph ✅) + Django App 合并优化 ✅ + 前端 API 文件合并 ✅ + 服务层/注册表/EventBus/守护进程 ✅
...
| **Agent (Agent)** | 运行 GAF 的机器实例，每台机器一个 Agent 进程 | 本机 Agent、远程 Agent-1 |
| **窗口 (Device 模型)** | 被控制的游戏窗口，分模拟器窗口和非模拟器窗口两类 | BD2 窗口、雷电模拟器 1 |
| **轮换规则 (RotationRule)** | 账户轮换策略 | 按时间切换、按任务完成切换 |

> **命名说明**：代码中 `Device` 模型实际代表的是"窗口"（Window 或 模拟器实例），而非"运行 GAF 的机器"。运行 GAF 的机器是 `Agent` 的概念。每台机器运行一个 Agent 进程，Agent 自动发现本机所有窗口（Windows 窗口 + 模拟器实例）并注册为 Device 记录。
...
GameAccount (游戏账户)
  ...
  └── M2M ↔ Task (任务) ← 通过 TaskGameAccount 中间表 ✅ 前后端完整

Task (任务)
  ├── FK → ResourcePack (资源包) ← N197-8: 任务直接关联资源包，运行时通过此资源包加载服务器对应模板
  ├── M2M ↔ GameAccount (游戏账户) ← 任务可绑定多个账户 ✅
  ├── FK → GameProfile (游戏档案, 归属过滤) ✅ 阶段 1
  └── FK → GameAccountRotation (轮换规则，可选) ✅

GameProfile (游戏档案) — 中心枢纽 ✅ 阶段 1 v3
  ├── FK → TaskChain (default_routine, 默认任务链) ✅ 阶段 1
  ├── default_screenshot_method (默认截图方式, 'auto' 继承) ✅ 阶段 1
  ├── default_input_method (默认输入方式, 'auto' 继承) ✅ 阶段 1
  └── default_control_mode (默认控制模式, 'auto' 继承) ✅ 阶段 1

TaskChain (任务链) ✅ 阶段 1 v3
  ├── FK → GameProfile (game_profile, 归属游戏档案) ✅
  ├── is_default (BooleanField, 标记为该 GameProfile 的默认链) ✅
  └── TaskChainNode (有序任务节点) ✅
      ├── node_type ('task' | 'pipeline') ✅ TD-110 Phase 1
      ├── FK → Task (task, nullable) ✅
      ├── FK → TaskChainNode (parent, nullable) ✅
      └── FK → Pipeline (pipeline, nullable) ✅ TD-110 Phase 1 — Pipeline 一等公民
          (node_type='pipeline' 时引用 Pipeline; node_type='task' 时引用 Task)

TaskChainExecution (任务链执行记录) ✅ 阶段 2 v3 新增
  ├── FK → TaskChain (任务链)
...
Device (设备)
  ├── FK → Agent (Agent) ← ✅ Phase R18: 自动关联
  ├── FK → GameProfile (game_profile, 绑定游戏档案) ✅ 阶段 1
  ├── FK → GameAccount (game_account, 运行时绑定当前账户) ✅ 阶段 1
  └── screenshot_method/input_method/control_mode ('auto' 时继承 GameProfile) ✅ 阶段 5

GameAccount (游戏账户)
  ├── FK → GameProfile (game_profile, 明确归属) ✅ 阶段 1
  └── FK → ResourcePack (resource_pack, 运行时资源包) ✅ Phase R16

TaskDevice (任务设备映射) ← 中间表
  └── M2M ↔ Task ↔ Device ← 任务可指定在多个设备上执行 ✅
  (注: is_default 字段已在阶段 1 删除 — 死字段, 执行链路不读取)

TaskExecution (执行记录)
  ├── FK → Task (任务)
  ├── FK → GameAccount (账户, 运行时绑定) ✅ 阶段 1
  ├── FK → Device (设备, 运行时绑定) ✅ 阶段 1
  └── FK → TaskChainExecution (任务链执行, nullable) ✅ 阶段 2 v3
```
...
| 关系 | 类型 | 状态 | 说明 |
|------|------|------|------|
| GameProfile → TaskChain | FK (default_routine) | ✅ 阶段 1 v3 | 游戏档案的默认任务链, 新窗口绑 GameProfile 自动继承 |
| TaskChain → GameProfile | FK (game_profile) | ✅ 阶段 1 v3 | 任务链归属游戏档案 |
| TaskChain.is_default | BooleanField | ✅ 阶段 1 v3 | 每 GameProfile 下最多一个 is_default=True (clean() 校验) |
| Task → ResourcePack | FK (多对一) | ✅ N197-8 | 任务直接关联资源包，运行时通过此资源包加载服务器对应模板；`ResourcePack.get_templates_for_server(server_region)` 按服务器分区选择模板 |
| GameAccount → ResourcePack | FK (多对一) | ✅ 已实施 | 账户级别默认资源包，供 Task.resource_pack 未设置时回退使用 |
| GameAccount → GameProfile | FK (game_profile) | ✅ 阶段 1 v3 | 明确归属, 替代 game_name 字符串弱关联 |
| Task ↔ GameAccount | M2M (多对多) | ✅ 已完成 | 前后端完整：任务可绑定多个账户 |
| Task ↔ Device | M2M (多对多) | ✅ 已完成 | TaskDevice 中间表（is_default 已删除 — 死字段） |
| Device → Agent | FK (多对一) | ✅ Phase R18 | Agent 启动后自动发现并关联设备 |
| Device → GameProfile | FK (game_profile) | ✅ 阶段 1 | 设备绑定游戏档案 |
| Device → GameAccount | FK (game_account) | ✅ 阶段 1 | 运行时绑定当前执行的账户 |
| Task → RotationRule | FK (多对一) | ✅ 已有 | 任务可选轮换规则 |
| TaskExecution → Task/Account/Device | FK (多对一) | ✅ 已有 | 记录每次执行的完整上下文 |
| TaskExecution → TaskChainExecution | FK (nullable) | ✅ 阶段 2 v3 | 追踪 TaskChain 派发, nullable 兼容单 Task 执行 |
| TaskChainExecution → TaskChain/Device/GameAccount | FK | ✅ 阶段 2 v3 | TaskChain 派发追踪 (1:N → TaskExecution) |
| DeviceResourceMapping | 中间表 | ✅ 已删除 | Phase R16 中已移除 |

### 3.3 当前后端模型

```python
# ✅ Task 模型 (阶段 1 v3 后 + N197-8)
class Task:
    resource_pack = models.ForeignKey(ResourcePack, ...)    # ✅ N197-8: 任务直接关联资源包，运行时加载对应服务器模板
    game_accounts = models.ManyToManyField(GameAccount, ...)  # ✅ 保留 (可选账户白名单)
    game_profile = models.ForeignKey(GameProfile, ...)        # ✅ 保留 (归属过滤；2026-07-14 起 GameProfile 详情页支持 bind/unbind)
    rotation_rule = models.ForeignKey(RotationRule, ...)      # ✅ 保留 (执行链路在用)
    # game_account (单 FK) 已移除 → M2M game_accounts 已替代

# ✅ GameProfile 模型 (阶段 1 v3 后) — 中心枢纽
class GameProfile:
    default_routine = models.ForeignKey('TaskChain', ...)     # ✅ v3 新增: 默认任务链
    default_screenshot_method = models.CharField(...)         # ✅ v3 新增: 'auto' 继承
    default_input_method = models.CharField(...)              # ✅ v3 新增: 'auto' 继承
    default_control_mode = models.CharField(...)              # ✅ v3 新增: 'auto' 继承
...
# ✅ TaskChain 模型 (阶段 1 v3 后)
class TaskChain:
    game_profile = models.ForeignKey(GameProfile, ...)        # ✅ v3 新增: 归属游戏档案
    is_default = models.BooleanField(default=False)           # ✅ v3 新增: 标记默认链
    def clean(self): ...                                      # ✅ v3 新增: 校验每 GameProfile 最多一个 is_default

# ✅ TaskChainExecution 模型 (阶段 2 v3 新增) — TaskChain 派发追踪
...
# ✅ GameAccount 模型 (阶段 1 v3 后)
class GameAccount:
    resource_pack = models.ForeignKey(ResourcePack, ...)      # ✅ 回退: 账户级别默认资源包, Task.resource_pack 未设置时使用
    game_profile = models.ForeignKey(GameProfile, ...)        # ✅ v3 新增: 明确归属
    # allowed_resource_packs (M2M) 已移除 → 死字段, 执行不校验

# ✅ TaskDevice 中间表 (阶段 1 后)
class TaskDevice:
...
class Device:
    ...
    game_profile = models.ForeignKey(GameProfile, ...)        # ✅ v3: 绑定游戏档案
    game_account = models.ForeignKey(GameAccount, ...)        # ✅ v3: 运行时绑定当前账户
    # screenshot_method/input_method/control_mode: 'auto' 时继承 GameProfile (阶段 5)

# ✅ TaskExecution 模型 (阶段 2 v3 后)
class TaskExecution:
    task = models.ForeignKey(Task, ...)
    game_account = models.ForeignKey(GameAccount, ...)        # ✅ 阶段 1: 运行时账户绑定
    device = models.ForeignKey(Device, ...)                   # ✅ 阶段 1: 运行时设备绑定
    task_chain_execution = models.ForeignKey(TaskChainExecution, null=True, ...)  # ✅ 阶段 2 v3
```
...
|:---:|:---:|:---|:---:|:---|:---|
| **emulator** | EmulatorDiscovery | `127.0.0.1:5555` | ✅ 可最小化 | ADB（截图/输入不依赖窗口前台） | 雷电/MuMu/夜神等模拟器 (DeviceType.EMULATOR)。最小化后仍可通过 ADB 控制 |
| **windows** | WindowDiscovery | - | ❌ 视控制模式而定 | Win32 API | Windows 游戏窗口 (DeviceType.WINDOWS)。控制模式决定前台依赖程度 |
| **android** | ADB devices | `emulator-5554` | N/A | ADB | 真机通过 ADB 访问，归入 EMULATOR 类型，无独立枚举 |
| **ios** | 未来扩展 | - | N/A | ❌ 待实现 | iOS 设备 (待实现) |
...
| [tasks](file:///d:/code/GAF/backend/tasks) | 任务/执行/调度/标签/文件夹 | Task, TaskExecution, ScheduledTask, TaskVersion, TaskFolder, Tag |
...
| [resources](file:///d:/code/GAF/backend/resources) | 资源包/模板/标注 | ResourcePack, Template, TemplateVersion, TemplateAnnotation, Tag |
| [agents](file:///d:/code/GAF/backend/agents) | Agent/Device/DeviceGroup | Agent, Device, DeviceGroup |
...
| [executions](file:///d:/code/GAF/backend/executions) | 执行管理视图层 (无自有模型，视图读取 TaskExecution/TaskStep) + 分析报表端点 (`/executions/*` + `/analytics/*`) | (无 Model) |

> 注: TaskExecution CRUD 视图集 (task-executions) 已于 2026-08-04 迁入 `tasks` app (`tasks/execution_views.py`)；`executions` app 仍保留执行管理视图层 (steps/intervene/analysis/daily-report/unattended-logs/trend + analytics 5 端点)，无自有模型。
...
| App | 职责 | 关键模型 |
|-----|------|---------|
| [debug](file:///d:/code/GAF/backend/debug) | 崩溃报告/日志归档/LLM 分析 | CrashReport, DebugLogArchive, BackupRecord, LLMAnalysisResult |
...
| 后台截图 | ✅ agent WGC/PrintWindow (backend WGC mock 删除 TD-125, DXGI 支持 hwnd crop TD-124) | ⚠️ CGWindow 有限 | ⚠️ XShm 有限 | ✅ scrcpy/NemuIpc |
| 后台输入 | ✅ PostMessage (client 坐标, Spec B TD-122); SendInput/PseudoBackground 串行化 (Spec C TD-121) | ❌ 需辅助功能权限 | ⚠️ XSendEvent | ✅ minitouch/MaaTouch (动态端口分配, Spec D TD-123) |
| 伪最小化 | ✅ | ❌ 不需要 | ❌ 不需要 | ❌ 不需要 |
| 模拟器发现 | ✅ 注册表+进程 | ⚠️ 进程+ADB | ⚠️ 进程+ADB | ✅ ADB devices |
| Multi-game 模式约束 | ✅ FeatureFlag `unattended_multi_game_mode` + 白名单降级 (Spec A) | ❌ 不适用 | ❌ 不适用 | ✅ 同 Windows 白名单 |
| OCR | ✅ 全引擎 | ✅ PaddleOCR/RapidOCR | ✅ PaddleOCR/RapidOCR | — |
| ONNX 推理 | ✅ DirectML/CUDA | ✅ CoreML/CPU | ✅ CUDA/CPU | — |
...
| 模块 | 状态 | 说明 |
|------|------|------|
| 认证系统 | ✅ | JWT + 2FA + OAuth + API Key |
| 设备管理 | ✅ | Windows + Android + 模拟器 + 自动关联 |
| 任务管理 | ✅ | CRUD + Pipeline 编辑 + 录制 + 市场 |
| 资源包管理 | ✅ | 导入/导出 + 模板库 + 标注 |
| 游戏账户 | ✅ | CRUD + 分组 + 轮换 + AES 加密 |
| 调度中心 | ✅ | 定时任务 + 时间窗口 + 无人值守 + 5 层恢复 (task 级 + step 级信号接入 P-010) |
| TaskChain 编排 | ✅ | DAG editor + Pipeline 一等公民节点 (TD-110) + routine.json → TaskChain 自动转换 |
| 监控告警 | ✅ | 规则 + 事件 + 诊断 + 告警升级 (P1→P0) |
| AI 模块 | ✅ | 助手 + QA + Skill + LangGraph Agent + RAG |
| 跨平台层 | ✅ | Windows 完整 (agent 侧 `agent/src/platforms/windows/`) + macOS/Linux 已落地 (backend 侧 `backend/device_bridge/platforms/{macos,linux}/`, P-028); 注: agent 侧仅 Windows, macOS/Linux 在 backend 侧 |
| 运维监控 | ✅ | 链路追踪 + SLA 指标 + 崩溃报告 + 日志归档 |
| **审计日志 (C-045)** | ✅ | AuditMixin (gaf_core/mixins/audit.py) — 114 接入点, 19 个 ViewSet 自动写 AuditLog (accounts/tasks/agents/pipeline/resources/qa/notifications/scheduler/settings/gamestate/debug/protocol/monitors/plugins); AuditLog 模型在 `accounts/models.py:454` |
| 通知系统 | ✅ | 7 渠道 (邮件/Webhook/钉钉/飞书/企微/Telegram/自定义) |
| 插件系统 | ✅ | 上传/安装/启停/沙箱 |
| 国际化 | ✅ | 4 语言 (en/zh_Hans/ja/ko) |
| **服务层 (Phase 1)** | ✅ | TaskExecutor 统一执行入口 + TaskService/ExecutionService 服务层子包 |
| **设备发现注册表 (Phase 2)** | ✅ | DeviceDiscoveryRegistry 注册表驱动发现，统一 ADB/Windows/模拟器发现 |
| **统一 LLM 客户端 (Phase 2)** | ✅ | BaseLLMClient ABC + WebSocket RPC，agent 和 backend 统一客户端 |
...
| 设备资源映射表 | ~~DeviceResourceMapping 存在~~ | 已删除 | ✅ 已修复 (R16) |
| 前端绑定 UI | 只有资源包绑定 | 账户/设备绑定 | ✅ 已修复 (R16) |
| 任务-账户绑定 | 后端有 M2M，前端无 UI | 补全绑定 UI | ✅ 已修复 (R16) |

### 13.3 缺失的前端 UI（已补全）

| 功能 | 后端 | 前端 | 说明 |
|------|------|------|------|
| 任务绑定资源包 | ✅ FK | ✅ 已完成 | TaskFormModal 支持 resource_pack 选择；任务列表显示资源包列 + 下拉筛选 |
| 任务绑定账户 | ✅ M2M | ✅ 已完成 | TaskFormModal 支持 game_accounts 多选 |
| 任务分配设备 | ✅ TaskDevice | ✅ 已完成 | TaskFormModal 支持 devices 多选 |
| 账户绑定资源包 | ✅ FK | ✅ 已完成 | GameAccountEditor 支持 resource_pack 选择 |
...
| 决策 | 内容 | 原因 |
|------|------|------|
| 轮换双入口 + 循环轮换 (TD-400) | ✅ 2026-08-26 已实施 | 任务级轮换（每次执行按 `rotation_rule` 全账户矩阵选取）适合"定时跑一遍"；无人值守级轮换（`session.rotation_rule` + `loop_rotation`）适合"持续循环挂机"——链完成后账户归还池子继续派发，循环模式不触发 `all_completed` 自动停止 |
| 资源包绑在任务上（主）+ 回退到账户（辅） | ✅ N197-8: Task.resource_pack 直接关联；`get_templates_for_server(server_region)` 按服务器分区加载 | 任务运行时加载模板的依据；换服务器只需改任务绑定的资源包，同一任务可被多个服务器共用 |
| 设备不绑资源包 | ✅ 已实施 | 设备只负责执行方式，与游戏无关 |
| 轮换按账户维度 | ✅ | 一个账户所有任务完成后再换下一个 |
| 设备并行执行 | ✅ | 多开模拟器可同时跑不同账户 |
| TaskDevice 中间表 | ✅ 新增 | 任务指定执行设备，替代原 DeviceResourceMapping |
...
| PostMessage client 坐标 (Spec B / TD-122) | ✅ 2026-07-16 | 4 个非 scroll PostMessage 方法移除 `_client_to_screen`, 直接 pack client 坐标 (WM_LBUTTONDOWN/UP/MOUSEMOVE lParam 期望 client) |
| SendInput/PseudoBackground 串行化 (Spec C / TD-121) | ✅ 2026-07-16 | `WindowsInputHandler` 实例级 `threading.RLock` 串行化 6 个方法 (RLock 因 PseudoBackground 内部调 `_sendinput` 需重入) |
| minitouch/MaaTouch 动态端口 (Spec D / TD-123) | ✅ 2026-07-16 | per-serial CRC32 哈希端口分配 + 线性探测 (minitouch [11111, 11611), maatouch [13113, 13613)); 同 serial 同端口 |
| DXGI per-window crop (Spec E / TD-124) | ✅ 2026-07-16 | `DXGICapture.capture_window(hwnd)` — GetWindowRect + DesktopCoordinates 平移 + numpy slice + 边界 clip |
| Backend WGC mock 移除 (Spec E / TD-125) | ✅ 2026-07-16 | 删除 `_wgc.py` mock; `_capture_wgc` delegate 到 PrintWindow; `WINDOWS_METHODS` 移除 'WGC'; `MULTI_GAME_SAFE_SCREENSHOT_METHODS` 移除 'wgc' |
| 两套 WebSocket | ✅ 已实施 | agents (同步+ACK) + protocol (异步+帧协议) |
| GameProfile 聚合根 bind/unbind 端点 | ✅ 2026-07-14 | 详情页 4 Tab 管理子资源绑定关系；资源包 Tab 只读（绑 Account，架构 §3.2） |
| ScreenState 移除 | ✅ 2026-07-13 | 死代码清理：ScreenState/ScreenStateTransition 模型 + 前端编辑器全删 |
| 前端 UI 归一化 (C-026) | ✅ 2026-07-13 | 34 个 Navigate → 2 个；ops 9→7 项；system 8→7 项；概念归一化（日志=/ops/logs，LLM 分析=/ai/log-analysis） |
